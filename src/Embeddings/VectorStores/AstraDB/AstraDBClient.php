...
class AstraDBClient
{
    ...
    {
        if (! $endpoint && (getenv('ASTRADB_ENDPOINT') !== '' || ! empty($_ENV['ASTRADB_ENDPOINT']))) {
            $endpoint = (string) (getenv('ASTRADB_ENDPOINT') ?: $_ENV['ASTRADB_ENDPOINT']);
        }

        $this->endpoint = $endpoint ?? throw new \Exception('You have to provide a ASTRADB_ENDPOINT env var to connect to AstraDB.');

        if (! $token && (getenv('ASTRADB_TOKEN') !== '' || ! empty($_ENV['ASTRADB_TOKEN']))) {
            $token = (string) (getenv('ASTRADB_TOKEN') ?: $_ENV['ASTRADB_TOKEN']);
        }
